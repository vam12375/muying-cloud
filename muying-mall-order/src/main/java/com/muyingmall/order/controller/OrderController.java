...
public class OrderController {
    ...
    public Result<Map<String, Object>> createOrder(@RequestBody @Valid OrderCreateDTO orderCreateDTO) {
        ...
        try {
            ...
            return Result.success("创建成功", orderInfo);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public Result<Map<String, Object>> getOrderDebugStatus() {
        ...
        try {
            ...
            for (Order order : orders) {
                com.muyingmall.common.enums.OrderStatus oldStatus = order.getStatus();
                if (oldStatus == null) {
                    order.setStatus(com.muyingmall.common.enums.OrderStatus.PENDING_PAYMENT);
                    orderService.updateById(order);
                    fixedCount++;
                } else {
                    String statusCode = oldStatus.getCode();
                    if (statusCode != null && !statusCode.equals(statusCode.toLowerCase())) {
                        order.setStatus(com.muyingmall.common.enums.OrderStatus.fromCode(statusCode.toLowerCase()));
                        orderService.updateById(order);
                        fixedCount++;
                    }
                }
            ...
            for (Order order : orders) {
                com.muyingmall.common.enums.OrderStatus status = order.getStatus();
