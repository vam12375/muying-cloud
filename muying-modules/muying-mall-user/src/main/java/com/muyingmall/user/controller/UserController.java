...
import com.muyingmall.user.dto.LoginResponseDTO;
import com.muyingmall.user.dto.UserDTO;
...
public class UserController {
    ...
    public Result<LoginResponseDTO> login(@RequestBody @Valid LoginDTO loginDTO) {
        ...
        if (user == null) {
            user = userService.getByEmail(loginDTO.getUsername());
        }
    ...
    public Result<Void> logout() {
        // 对于无状态JWT，注销主要在客户端完成（删除令牌）。
        // 服务端可以实现令牌黑名单机制来强制令牌失效。
